...
image=PhotoImage(file="E:\\PROJECTS\\LBS - DRISHTI\\Drishti\\Bg_image.png")
...
message = tk.Label(window, text="EXAM SURVEILLANCE SYSTEM" ,bg ="darkblue"  ,fg="white"  ,width=30  ,height=1,font=('Times New Roman', 25, 'bold underline')) 
...
lbl2 = tk.Label(window, text="Enter Time Limit   :",width=20  ,fg="white"  ,bg="darkblue"    ,height=1 ,font=('Times New Roman', 25, ' bold ')) 
lbl2.place(x=230-x_cord, y=478-y_cord)

tm = tk.Entry(window,width=30  ,bg="white"  ,fg="black",font=('Times New Roman', 15, ' bold ')  )
...
def start_exam():
    ...
    def Fail(timee, redcard):
        if redcard >= 4:
            tk.messagebox.showinfo(title="BLACK-LIST", message=f"You have been caught cheating {redcard} times, You are responsible for punishment")
            window.destroy() 
    ...
    def main(args):
        ...
        if not timee:
            ...
            message.configure(text = res)
            MsgBox = tk.messagebox.askquestion ("Warning","Please enter time limit of exam in minutes",icon = 'warning')
            if MsgBox == 'no':
        ...
        while(webcam.isOpened()):
            ...
            # if redcard==6:   
            #     tk.messagebox.showinfo(title="ALERT", message=f"You have been caught cheating more than {redcard} times")
            #     window.destroy()

            print("<< Level of doubt:" , yellocard," || ", "Number of warnings:", redcard, " >>")
        ...
        # Draw a rectangle around the faces and predict the face name
            ...
            if cv2.waitKey(1) & 0xFF == ord('q'):
                ...
                name=(txt2.get())
                msg = name + " has been caught cheating"  + smsofsus(yellocard, redcard)
...
lbl5 = tk.Label(window, text="SURVEILLANCE" ,width=15  ,fg="white"  ,bg="darkblue"  ,height=1 ,font=('Times New Roman', 20, ' bold ')) 
...
trackImg = tk.Button(window, text="START EXAM", command=start_exam  ,fg="white"  ,bg="black"  ,width=25  ,height=2, activebackground = "white" ,font=('Times New Roman', 15, ' bold '))
